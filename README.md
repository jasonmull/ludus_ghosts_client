...
```yaml
    role_vars:
      # By default, the role sets the ApiRootUrl in the config\application.json file to the IP address of the GHOSTS server deployed in the range config.
      # If you'd like to set a static IP for the GHOSTS server, define the ghosts_config_replace value below to your GHOSTS server IP
